...
function Counter() {
  ...
  return (
    <>
      ...
      <span>
          {counter === 0 && `Today is ${date.toLocaleDateString('en-US', options)}`}
          {counter >=1   &&   `${counter} days from today is ${date.toLocaleDateString('en-US', options)} `}
          {counter < 0   &&   `${Math.abs(counter)} days ago was ${date.toLocaleDateString('en-US', options)}`}
      </span>
